  ...
  try {
    ...
    if (path.startsWith('/air/orders')) {
      if (method === 'POST') {
        ...
        // Generate a unique ID for each passenger using timestamp and random string
        const generatePassengerId = () => `pas_${Date.now()}${Math.random().toString(36).substring(2, 7)}`;
